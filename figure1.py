...
grid = plt.GridSpec(11, 2, wspace=0.3)

axs = [plt.subplot(grid[0, :]), plt.subplot(grid[4:7, 0]),
       plt.subplot(grid[4:7, 1]), plt.subplot(grid[8:, 0]),
       plt.subplot(grid[8:, 1]), plt.subplot(grid[2, 0]),
...
ax.add_patch(Rectangle((R2_t2-reftime, ymin), width, ymax-ymin, color='yellow',
                       alpha=0.5))
ax.set_title("R2/R3 zoom")
ax.set_xlabel("seconds relative to MQS P")
ax.text(labelxoff[2], labelyoff[2], 'C', transform=ax.transAxes, fontsize=14)
...
ax.text(labelxoff[3], labelyoff[3], 'D', transform=ax.transAxes, fontsize=14)
